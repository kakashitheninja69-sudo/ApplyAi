// FILE: src/components/resume-templates/SplitModern.tsx
import type { TemplateProps } from './shared'
import { accentHex, headingFont, formatDateRange } from './shared'

export default function SplitModern({ data }: TemplateProps) {
  const accent = accentHex(data)
  const hFont = headingFont(data)
  const { contact, work, education, skills, summary } = data

  const skillsByCategory: Record<string, typeof skills> = {
    Technical: skills.filter((s) => s.category === 'technical'),
    Tools: skills.filter((s) => s.category === 'tool'),
    Soft: skills.filter((s) => s.category === 'soft'),
    Languages: skills.filter((s) => s.category === 'language'),
  }

  return (
    <div
      style={{
        width: '794px',
        minHeight: '1123px',
        display: 'flex',
        fontFamily: 'Inter, sans-serif',
        fontSize: '11px',
        lineHeight: '1.5',
        color: '#1e293b',
        background: '#fff',
      }}
    >
      {/* Left sidebar 30% */}
      <div
        style={{
          width: '238px',
          background: accent,
          padding: '40px 22px',
          color: '#fff',
          flexShrink: 0,
          display: 'flex',
          flexDirection: 'column',
        }}
      >
        <h1
          style={{
            fontFamily: hFont,
            fontSize: '16px',
            fontWeight: 700,
            color: '#fff',
            margin: '0 0 4px 0',
            lineHeight: 1.3,
          }}
        >
          {contact.name || 'Your Name'}
        </h1>
        {contact.title && (
          <p style={{ fontSize: '10px', opacity: 0.85, fontWeight: 500, marginBottom: '24px' }}>
            {contact.title}
          </p>
        )}

        {/* Contact */}
        <SideSection label="Contact" />
        <div style={{ marginBottom: '20px', display: 'flex', flexDirection: 'column', gap: '5px' }}>
          {contact.email && (
            <p style={{ fontSize: '9px', opacity: 0.9, wordBreak: 'break-all', margin: 0 }}>
              ✉ {contact.email}
            </p>
          )}
          {contact.phone && (
            <p style={{ fontSize: '9px', opacity: 0.9, margin: 0 }}>📞 {contact.phone}</p>
          )}
          {contact.location && (
            <p style={{ fontSize: '9px', opacity: 0.9, margin: 0 }}>📍 {contact.location}</p>
          )}
          {contact.linkedin && (
            <p style={{ fontSize: '9px', opacity: 0.9, wordBreak: 'break-all', margin: 0 }}>
              🔗 {contact.linkedin}
            </p>
          )}
          {contact.website && (
            <p style={{ fontSize: '9px', opacity: 0.9, wordBreak: 'break-all', margin: 0 }}>
              🌐 {contact.website}
            </p>
          )}
        </div>

        {/* Skills */}
        {skills.length > 0 && (
          <>
            <SideSection label="Skills" />
            {Object.entries(skillsByCategory).map(([cat, list]) =>
              list.length > 0 ? (
                <div key={cat} style={{ marginBottom: '12px' }}>
                  <p
                    style={{
                      fontSize: '9px',
                      textTransform: 'uppercase',
                      letterSpacing: '0.08em',
                      opacity: 0.65,
                      marginBottom: '5px',
                      fontWeight: 600,
                    }}
                  >
                    {cat}
                  </p>
                  <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
                    {list.map((s) => (
                      <span
                        key={s.id}
                        style={{
                          background: 'rgba(255,255,255,0.2)',
                          borderRadius: '3px',
                          padding: '2px 6px',
                          fontSize: '9px',
                          fontWeight: 500,
                        }}
                      >
                        {s.name}
                      </span>
                    ))}
                  </div>
                </div>
              ) : null
            )}
          </>
        )}
      </div>

      {/* Right panel 70% */}
      <div style={{ flex: 1, padding: '40px 36px' }}>
        {summary && (
          <RSection accent={accent} hFont={hFont} label="Summary">
            <p style={{ color: '#475569', lineHeight: '1.7' }}>{summary}</p>
          </RSection>
        )}

        {work.some((w) => w.company || w.role) && (
          <RSection accent={accent} hFont={hFont} label="Experience">
            {work.map((w) => (
              <div key={w.id} style={{ marginBottom: '18px' }}>
                <div
                  style={{
                    display: 'flex',
                    justifyContent: 'space-between',
                    alignItems: 'baseline',
                  }}
                >
                  <p style={{ fontWeight: 700, fontSize: '12px', fontFamily: hFont }}>
                    {w.role || 'Role'}
                  </p>
                  <p style={{ fontSize: '10px', color: '#94a3b8' }}>
                    {formatDateRange(w.startDate, w.endDate, w.current)}
                  </p>
                </div>
                <p style={{ color: accent, fontWeight: 600, fontSize: '11px', marginBottom: '5px' }}>
                  {w.company}
                </p>
                <ul style={{ paddingLeft: '16px', display: 'flex', flexDirection: 'column', gap: '3px' }}>
                  {w.bullets.filter(Boolean).map((b, i) => (
                    <li key={i} style={{ listStyleType: 'disc', color: '#475569' }}>
                      {b}
                    </li>
                  ))}
                </ul>
              </div>
            ))}
          </RSection>
        )}

        {education.some((e) => e.institution) && (
          <RSection accent={accent} hFont={hFont} label="Education">
            {education.map((e) => (
              <div
                key={e.id}
                style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '10px' }}
              >
                <div>
                  <p style={{ fontWeight: 700, fontSize: '12px', fontFamily: hFont }}>{e.institution}</p>
                  <p style={{ color: '#64748b' }}>
                    {[e.degree, e.field].filter(Boolean).join(', ')}
                    {e.gpa ? ` — GPA ${e.gpa}` : ''}
                  </p>
                </div>
                <p style={{ fontSize: '10px', color: '#94a3b8', whiteSpace: 'nowrap' }}>
                  {formatDateRange(e.startDate, e.endDate, false)}
                </p>
              </div>
            ))}
          </RSection>
        )}
      </div>
    </div>
  )
}

function SideSection({ label }: { label: string }) {
  return (
    <div style={{ marginBottom: '8px' }}>
      <p
        style={{
          fontSize: '9px',
          textTransform: 'uppercase',
          letterSpacing: '0.1em',
          fontWeight: 700,
          color: 'rgba(255,255,255,0.7)',
          marginBottom: '4px',
        }}
      >
        {label}
      </p>
      <div style={{ height: '1px', background: 'rgba(255,255,255,0.25)', marginBottom: '10px' }} />
    </div>
  )
}

function RSection({
  accent,
  hFont,
  label,
  children,
}: {
  accent: string
  hFont: string
  label: string
  children: React.ReactNode
}) {
  return (
    <div style={{ marginBottom: '22px' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
        <p
          style={{
            fontFamily: hFont,
            fontWeight: 700,
            fontSize: '12px',
            color: accent,
            textTransform: 'uppercase',
            letterSpacing: '0.06em',
            margin: 0,
          }}
        >
          {label}
        </p>
        <div style={{ flex: 1, height: '1px', background: `${accent}30` }} />
      </div>
      {children}
    </div>
  )
}
// ---END FILE---
